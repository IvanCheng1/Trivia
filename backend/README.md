...
- General:
  - Only applies if search term is included in the request
  - Returns JSON object with the success value, a list of question objects, number of total questions in the search, and the current category
...
- General:
  - Only applies if search term is `NULL`
  - Returns JSON object with the success value, a list of current question objects, (updated) total number of questions, and the new question id
- Sample: `curl http://127.0.0.1:5000/questions -X POST -H "Content-Type: application/json" -d '{ "question": "What is our planet called?", "answer": "Earth", "difficulty": 1, "category": "4" }'`
```
{
  "created": 24, 
  "questions": [
...
- General:
  ...
  - Input JSON request parameters of category and a list of previous questions' id
  - Returns JSON object with the success value, and the next random question that had not been played previously
    - If there are not more available questions, returns JSON object only with the success value
